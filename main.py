...
async def execute_trade(decision: Dict[str, Any], market_prices: Dict[str, Any], market: Dict[str, Any], target_open: float):
...
async def update_trades(current_prices: Dict[str, Any]):
    ...
    for trade in state["active_trades"]:
        ...
        if is_closed or market_expired:
            if trade.get("settlement_price_at_expiry") is None:
                trade["settlement_price_at_expiry"] = current_prices.get("chainlink") or current_prices.get("spot")
            ...
            # Auto-Settle fallback logic uses internally stored strike instead of regex matching
            if winning_index == -1:
                if not outcomes:
                     outcomes = [settings.POLYMARKET_UP_LABEL, settings.POLYMARKET_DOWN_LABEL]
                
                strike_price = trade.get("strike_price")
                sp = trade.get("settlement_price_at_expiry")
                if strike_price and sp:
                    is_up_win = sp > strike_price
...
async def update_loop():
    ...
    while True:
        try:
            ...
            if poly_snapshot["ok"]:
                await execute_trade(decision, poly_snapshot["prices"], poly_snapshot["market"], target_open)
