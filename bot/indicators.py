...
def count_consecutive_hist(hist_series: List[float]) -> Dict:
    ...
    # Direction is determined by the slope (rising vs falling)
    current_slope = last - prev
    target = "up" if current_slope > 0 else "down"

    count = 0
    for i in range(len(hist_series) - 1, 0, -1):
        curr_val = hist_series[i]
        prev_val = hist_series[i-1]
        if curr_val is None or prev_val is None or pd.isna(curr_val) or pd.isna(prev_val):
            break
        
        slope = curr_val - prev_val
